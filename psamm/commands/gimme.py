...
from ..fluxanalysis import FluxBalanceProblem, flux_randomization
...
class GimmeCommand(MetabolicMixin, LoopRemovalMixin, ObjectiveMixin,
    ...
    def run(self):
        ...
        threshold_dict, threshold_value = parse_transcriptome_file(self._args.transcriptome_file, self._args.expression_threshold)
        make_gimme_model(self._model, self._get_objective(), threshold_dict, threshold_value, self._args.biomass_threshold)
...
def make_gimme_model(model, biomass, transcriptome_dict, trasncript_values, objective_threshold):
	# print(transcriptome_dict)
	base_gene_dict = {}
	for rxn in model.reactions:
		base_gene_dict[rxn.id] = rxn.genes
	base_mm = model.create_metabolic_model()
	exchange_exclude = [rxn for rxn in base_mm.reactions if base_mm.is_exchange(rxn)]

	mm, reversible_gene_assoc, split_rxns = make_irreversible(model.create_metabolic_model(), base_gene_dict, exclude_list=exchange_exclude)
	ci_dict = {}
	...
	for reaction in mm.reactions:
		...
		if gene_string is None:
			...
			if ci is not None:
				# print(reaction, ci, gene_string, [trasncript_values.get(x.symbol) for x in e._variables])
				ci_dict[reaction] = ci
	...
	for (forward, reverse) in split_rxns:
		p.prob.add_linear_constraints(p.prob.var('zi_{}'.format(forward)) + p.prob.var('zi_{}'.format(reverse)) <= 1)

	if objective_threshold is None:
		p.maximize(biomass)
		p.prob.add_linear_constraints(p.get_flux_var(biomass) >= p.get_flux(biomass))
	else:
		p.maximize(biomass)
		logger.info('{}'.format(p.get_flux(biomass)))
		p.prob.add_linear_constraints(p.get_flux_var(biomass) >= objective_threshold)
	...
	for reaction in original_reaction_set:
		if reaction in used_rxns:
			if reaction in below_threshold_ids:
				used_below += 1
				# print(reaction, 'used_below_threshold')
				final_model.append(reaction)
			else:
				# print(reaction, 'used_above_threshold')
				used_above += 1
				final_model.append(reaction)
		else:
			if reaction in below_threshold_ids:
				# print(reaction, 'off_below_threshold')
				off_below += 1
			else:
				# print(reaction, 'off_above_threshold')
				off_above += 1
				final_model.append(reaction)

	# for i in sorted(mm.reactions):
	# 	print(i, p.get_flux(i))
	# quit()
	logger.info('Inconsistency Score\t{}'.format(p.prob.result.get_value(obj)))
	logger.info('final model size\t{}'.format(len(final_model)))
	logger.info('reactions below expression threshold used/total {}/{}'.format(str(used_below), str(used_below+off_below)))
	logger.info('Biomass flux: {}'.format(p.get_flux(biomass)))
	for i in sorted(final_model):
		print(i)


def parse_transcriptome_file(file, threshold):
	threshold_value = {}
	threshold_dict = {}
...
def get_rxn_value(root, gene_dict):
	if type(root) == boolean.Variable:
		...
		val_list = [x for x in [get_rxn_value(i, gene_dict) for i in root._terms] if x is not None]
		if len(val_list) == 0:
			return None
		else:
		...
		val_list = [x for x in [get_rxn_value(i, gene_dict) for i in root._terms]]
		if None in val_list:
...
def make_irreversible(mm, gene_dict, exclude_list = [], all_reversible = False):
