...
class GimmeCommand(MetabolicMixin, LoopRemovalMixin, ObjectiveMixin,
	...
	def run(self):
		...
		final_model, below_threshold_ids, incon_score = solve_gimme_problem(p, mm_irreversible,
		                                            self._get_objective(), reversible_gene_assoc, split_rxns,
		                                            threshold_value, self._args.biomass_threshold)

		for reaction in sorted(final_model):
			print(reaction)

		logger.info('Used {} below threshold reacctions out of {} total below threshold reactions'.format(
				str(len([x for x in below_threshold_ids if x in final_model])), str(len(below_threshold_ids))))
...
def solve_gimme_problem(problem, mm, biomass, reversible_gene_assoc, split_rxns, transcript_values, threshold):
	...
	if threshold is None:
		...
		if problem.get_flux(biomass) < threshold:
			logger.warning('Input threshold greater than maximum biomass: {}'.format(problem.get_flux(biomass)))
		else:
			problem.prob.add_linear_constraints(
...
def parse_transcriptome_file(f, threshold):
	...
	for row in csv.reader(f, delimiter=str('\t')):
		try:
			...
			logger.warning('Invalid expression value provided: gene: {} value: {}'.format(row[0], row[1]))
