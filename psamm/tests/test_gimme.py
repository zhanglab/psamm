...
class TestAddReactions(unittest.TestCase):
	...
	def test_reverse_model(self):
		...
		mm_irreversible, reversible_gene_assoc, split_rxns = gimme.make_irreversible(
			self._mm, self._assoc, exclude_list=['ex_A', 'rxn_6'])
		self.assertEqual(split_rxns, set([('rxn_4_forward', 'rxn_4_reverse'), ('rxn_3_forward', 'rxn_3_reverse')]))
		self.assertEqual(reversible_gene_assoc, test_associations)
		self.assertEqual(set([i for i in mm_irreversible.reactions]), {'rxn_1', 'rxn_2', 'rxn_3_forward', 'rxn_3_reverse', 'rxn_4_forward', 'rxn_4_reverse', 'rxn_5', 'rxn_6', 'ex_A'})

	def test_parse_transcriptome_file(self):
		f = ['gene\texpression', 'gene_1\t15', 'gene_2\t20', 'gene_3\t15', 'gene_4\t25', 'gene_5\t10', 'gene_6\t25']
	...
	def test_get_rxn_value(self):
		...
		f = ['gene\texpression', 'gene_1\t15', 'gene_2\t20', 'gene_3\t15', 'gene_4\t25', 'gene_5\t10', 'gene_6\t25']
		d = gimme.parse_transcriptome_file(f, 20)
		root_single = gimme.get_rxn_value(boolean.Expression(reversible_gene_assoc['rxn_1'])._root, d)
		self.assertEqual(root_single, 5)

		root_and = gimme.get_rxn_value(boolean.Expression(reversible_gene_assoc['rxn_5'])._root, d)
		self.assertEqual(root_and, 10)

		root_none = gimme.get_rxn_value(boolean.Expression(reversible_gene_assoc['rxn_2'])._root, d)
	...
	def test_gimme_model(self):
		...
		final_model, below_threshold_ids, incon_score = gimme.solve_gimme_problem(p, mm_irreversible,
		                                                                    'rxn_6',reversible_gene_assoc, split_rxns,
		                                                                    threshold_dict, 20)
