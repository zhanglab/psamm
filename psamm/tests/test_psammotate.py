...
class TestPsammotate(unittest.TestCase):
	...
	def test_app_reader(self):
		translation_dict = psammotate.app_reader(open(os.path.join(self._model_dir, 'app.tsv')), 1, 0)
	...
	def test_model_loader_ignorena(self):
		translation_dict = psammotate.app_reader(open(os.path.join(self._model_dir, 'app.tsv')), 1, 0)
		translated_genes = psammotate.model_loader(self._model, True, translation_dict)
		self.assertEqual(translated_genes[self.rxn_entry_dict['rxn_1']], ['gene_1', 'gene_a', True])
		self.assertEqual(translated_genes[self.rxn_entry_dict['rxn_2']], ['gene_2 and gene_3', 'gene_b and gene_c', True])
		self.assertEqual(translated_genes[self.rxn_entry_dict['rxn_3']], ['gene_3 and gene_4', 'gene_c and -', False])
		self.assertEqual(translated_genes[self.rxn_entry_dict['rxn_4']], ['gene_3 or gene_4', 'gene_c or -', True])
		self.assertEqual(translated_genes[self.rxn_entry_dict['rxn_5']], ['(gene_3 or gene_4) and (gene_1 or gene_5)',
		                                                                  '(gene_c or -) and (gene_a or (gene_e or gene_f))', True])
		self.assertEqual(translated_genes[self.rxn_entry_dict['rxn_7']], [None, None, False])
		self.assertEqual(translated_genes[self.rxn_entry_dict['rxn_8']], [None, None, False])

	def test_model_loader(self):
		translation_dict = psammotate.app_reader(open(os.path.join(self._model_dir, 'app.tsv')), 1, 0)
		translated_genes = psammotate.model_loader(self._model, False, translation_dict)
		self.assertEqual(translated_genes[self.rxn_entry_dict['rxn_1']], ['gene_1', 'gene_a', True])
		self.assertEqual(translated_genes[self.rxn_entry_dict['rxn_2']], ['gene_2 and gene_3', 'gene_b and gene_c', True])
		self.assertEqual(translated_genes[self.rxn_entry_dict['rxn_3']], ['gene_3 and gene_4', 'gene_c and -', False])
		self.assertEqual(translated_genes[self.rxn_entry_dict['rxn_4']], ['gene_3 or gene_4', 'gene_c or -', True])
		self.assertEqual(translated_genes[self.rxn_entry_dict['rxn_5']], ['(gene_3 or gene_4) and (gene_1 or gene_5)',
		                                                                  '(gene_c or -) and (gene_a or (gene_e or gene_f))', True])
		self.assertEqual(translated_genes[self.rxn_entry_dict['rxn_7']], [None, None, True])
		self.assertEqual(translated_genes[self.rxn_entry_dict['rxn_8']], [None, None, True])

	def test_get_gene_list_and(self):
		genes = 'gene_a and gene_b'
		self.assertEqual(psammotate.get_gene_list(genes), frozenset(['gene_a', 'gene_b']))
	...
	def test_get_gene_list_nested_or(self):
		...
		self.assertEqual(psammotate.get_gene_list(genes), frozenset(['gene_a', 'gene_b', 'gene_c']))
	...
	def test_get_gene_list_nested_and_or(self):
		...
		self.assertEqual(psammotate.get_gene_list(genes), frozenset(['gene_a', 'gene_b', 'gene_c', 'gene_d']))
