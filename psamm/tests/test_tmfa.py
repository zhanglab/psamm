...
class TestTMFA(unittest.TestCase):
    ...
    def test_parsedgr(self):
        ...
        self.assertEqual(dgr_dict, {'rxn_1': (Decimal(1), Decimal(2)),
            'rxn_2_forward': (Decimal(-4), Decimal(0.5)),
            'rxn_2_reverse': (Decimal(4), Decimal(0.5))})
...
class TestSolving(unittest.TestCase):
    def setUp(self):
        ...
        self.prob, self.cpd_xij_dict = tmfa.add_conc_constraints(
            self.xij, self.prob, {}, self.cp_list, ['h2o[c]',
                'h2o[e]'], 'h[c]', 'h[e]', '')
        self.prob, self.excluded_compounds = tmfa.add_reaction_constraints(
            self.prob, self.v, self.zi, self.dgri, self.xij,
            self.mm_irreversible, [],
            ['EX_A', 'EX_C', 'EX_h', 'EX_h2o', 'bio'],
            ['EX_A', 'EX_C', 'EX_H', 'EX_h2o', 'bio'], self.dgr_dict,
        ...
        self.ham_prob, self.ham_excluded_compounds = \
            ...
            self.ham_zi, self.ham_dgri, self.ham_xij, self.mm_irreversible,
            [], ['EX_A', 'EX_C', 'EX_h', 'EX_h2o', 'bio'],
            ['EX_A', 'EX_C', 'EX_H', 'EX_h2o', 'bio'], self.dgr_dict,
             {}, self.split_reversible, self.trans_param,
             [i for i in self.mm_irreversible.reactions],
    ...
    def test_addconc_constraints_default(self):
        prob, cpd_xij_dict = tmfa.add_conc_constraints(self.empty_xij,
            self.empty_prob, {}, self.empty_cp_list,
            ['h2o[c]', 'h2o[e]'], 'h[c]', 'h[e]', '')
        prob, excluded_compounds = tmfa.add_reaction_constraints(
            prob, self.empty_v, self.empty_zi, self.empty_dgri,
            self.empty_xij, self.mm_irreversible, [],
            ['EX_A', 'EX_C', 'EX_h', 'EX_h2o', 'bio'],
            ['EX_A', 'EX_C', 'EX_H', 'EX_h2o', 'bio'], self.dgr_dict,
             {}, self.split_reversible, self.trans_param,
             [i for i in self.mm_irreversible.reactions],
        ...
        for cpd in ['A[c]', 'B[c]', 'C[c]']:
            cpd_range_dict[cpd] = (tmfa.get_var_bound(prob,
                self.empty_xij[cpd], lp.ObjectiveSense.Minimize),
                tmfa.get_var_bound(prob, self.empty_xij[cpd],
                lp.ObjectiveSense.Maximize))

        self.assertAlmostEqual(cpd_range_dict['A[c]'][0],
            (-11.512925026140119, -3.912023005428146)[0], places=6)
        self.assertAlmostEqual(cpd_range_dict['A[c]'][1],
            (-11.512925026140119, -3.912023005428146)[1], places=6)
    ...
    def test_addconc_constraints_nondefault(self):
        prob, cpd_xij_dict = tmfa.add_conc_constraints(self.empty_xij,
            self.empty_prob, {'A[c]': (0.005, 0.005), 'B[c]': (0.0005, 0.005)},
            self.empty_cp_list, ['h2o[c]', 'h2o[e]'], 'h[c]', 'h[e]', '')
        prob, excluded_compounds = tmfa.add_reaction_constraints(prob,
            self.empty_v, self.empty_zi, self.empty_dgri, self.empty_xij,
            self.mm_irreversible, [],
            ['EX_A', 'EX_C', 'EX_h', 'EX_h2o', 'bio'],
            ['EX_A', 'EX_C', 'EX_H', 'EX_h2o', 'bio'], self.dgr_dict,
             {}, self.split_reversible, self.trans_param,
        ...
        self.assertAlmostEqual(cpd_range_dict['A[c]'][0],
            (-5.298317366548036, -5.298317366548036)[0], places=6)
        self.assertAlmostEqual(cpd_range_dict['A[c]'][1],
            (-5.298317366548036, -5.298317366548036)[1], places=6)
        self.assertAlmostEqual(cpd_range_dict['B[c]'][0],
            (-7.600902459542082, -5.298317366548036)[0], places=6)
        self.assertAlmostEqual(cpd_range_dict['B[c]'][1],
            (-7.600902459542082, -5.298317366548036)[1], places=6)
        self.assertAlmostEqual(cpd_range_dict['C[c]'][0],
            (-11.512925026140119, -3.912023005428146)[0], places=6)
        self.assertAlmostEqual(cpd_range_dict['C[c]'][1],
            (-11.512925026140119, -3.912023005428146)[1], places=6)
    ...
    def test_dgrrange(self):
        ...
        for reaction in ['rxn1_forward', 'rxn4', 'rxn6']:
            dgr_dict[reaction] = (tmfa.get_var_bound(self.prob,
            self.dgri(reaction), lp.ObjectiveSense.Minimize),
            tmfa.get_var_bound(self.prob, self.dgri(reaction),
            lp.ObjectiveSense.Maximize))
        self.assertAlmostEqual(dgr_dict['rxn1_forward'][0],
            (9.999999974752427e-07, 69.28332553115047)[0], places=6)
        self.assertAlmostEqual(dgr_dict['rxn1_forward'][1],
            (9.999999974752427e-07, 69.28332553115047)[1], places=6)
        self.assertAlmostEqual(dgr_dict['rxn4'][0],
            (-39.64166326557521, -9.999999903698154e-07)[0], places=6)
        self.assertAlmostEqual(dgr_dict['rxn4'][1],
            (-39.64166326557521, -9.999999903698154e-07)[1], places=6)
        self.assertAlmostEqual(dgr_dict['rxn6'][0],
            (265.3583367344248, 334.6416632655752)[0], places=6)
        self.assertAlmostEqual(dgr_dict['rxn6'][1],
            (265.3583367344248, 334.6416632655752)[1], places=6)
    ...
    def test_cpd_range(self):
        ...
        for cpd in ['A[c]', 'A[e]', 'D[c]', 'E[c]']:
            cpd_dict[cpd] = (tmfa.get_var_bound(self.prob,
            self.xij(cpd), lp.ObjectiveSense.Minimize),
            tmfa.get_var_bound(self.prob, self.xij(cpd),
            lp.ObjectiveSense.Maximize))
        self.assertAlmostEqual(cpd_dict['A[c]'][1],
            (-11.512925464970229, -3.912023005428146)[1], places=6)
    ...
    def test_constrainedconc(self):
        ...
        for reaction in ['rxn1_forward', 'rxn4', 'rxn6']:
            ...
            lp.ObjectiveSense.Maximize))
        self.assertAlmostEqual(cpd_dict['D[c]'][1],
            (-11.51292546497023, -4.90292494313994)[1], places=6)
        self.assertAlmostEqual(cpd_dict['A[c]'][1],
            (-10.0, -10.0)[1], places=6)
        self.assertAlmostEqual(cpd_dict['B[c]'][1],
            (-6.0, -6.0)[1], places=6)
        self.assertAlmostEqual(cpd_dict['E[c]'][1],
            (-11.512925245555174, -3.912023005428146)[1], places=6)
        self.assertAlmostEqual(dgr_dict['rxn4'][1],
            (-30.125556943039474, -9.999999974752427e-07)[1], places=6)
    ...
    def test_reversibleflux(self):
        ...
        for reaction in ['rxn1_forward', 'rxn1_reverse',
        'rxn2_forward', 'rxn2_reverse']:
            flux_dict[reaction] = (tmfa.get_var_bound(self.prob,
            self.v(reaction), lp.ObjectiveSense.Minimize),
    ...
    def test_ham_flux(self):
        ...
        for reaction in ['rxn1_forward', 'rxn2_forward', 'rxn2_reverse']:
            flux_dict[reaction] = (tmfa.get_var_bound(self.ham_prob,
            self.ham_v(reaction), lp.ObjectiveSense.Minimize),
        ...
        dgr_dict = {}
        for reaction in ['rxn1_forward', 'rxn4', 'rxn6']:
            dgr_dict[reaction] = (tmfa.get_var_bound(self.ham_prob,
            self.ham_dgri(reaction), lp.ObjectiveSense.Minimize),
    ...
    def test_err_flux(self):
        ...
        for reaction in ['rxn1_forward', 'rxn2_forward', 'rxn2_reverse']:
            flux_dict[reaction] = (tmfa.get_var_bound(self.err_prob,
                self.err_v(reaction), lp.ObjectiveSense.Minimize),
        ...
        dgr_dict = {}
        for reaction in ['rxn1_forward', 'rxn4', 'rxn6']:
            dgr_dict[reaction] = (tmfa.get_var_bound(self.err_prob,
            self.err_dgri(reaction), lp.ObjectiveSense.Minimize),
        ...
        self.assertAlmostEqual(dgr_dict['rxn4'][0],
            (-43.64166326557521, -9.999999903698154e-07)[0], places=6)
        self.assertAlmostEqual(dgr_dict['rxn4'][1],
            (-43.64166326557521, -9.999999903698154e-07)[1], places=6)
        self.assertAlmostEqual(dgr_dict['rxn6'][0],
            (261.3583367344248, 338.6416632655752)[0], places=6)
        self.assertAlmostEqual(dgr_dict['rxn6'][1],
            (261.3583367344248, 338.6416632655752)[1], places=6)
