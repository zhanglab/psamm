...
class TestVariable(unittest.TestCase):
    ...
    def test_variable_substitute(self):
        self.assertEqual(Variable('x').substitute(
            lambda v: {'x': 567}.get(v.symbol, v)), 567)

    def test_variable_substitute_unknown(self):
        self.assertEqual(Variable('x').substitute(
            lambda v: v), Variable('x'))

    def test_variable_substitute_multiple(self):
        self.assertEqual(Variable('x').substitute(
            lambda v: {'y': 123, 'x': 56}.get(v.symbol, v)), 56)
    ...
    def test_variables_ordered_by_symbol(self):
        var_list = sorted([Variable('b'), Variable('cd'),
                           Variable('a'), Variable('cc')])
        self.assertEqual(var_list, [Variable('a'), Variable('b'),
                                    Variable('cc'), Variable('cd')])
...
class TestExpression(unittest.TestCase):
    ...
    def test_expression_substitute_with_variable(self):
        ...
        self.assertEqual(e.substitute(
            lambda v: {'y': Variable('x')}.get(v.symbol, v)), Expression('3x'))

    def test_expression_substitute_with_expression(self):
        e = Expression({Variable('x'): 3, Variable('y'): -2})
        es = e.substitute(
            lambda v: {'x': Expression('y + 2z')}.get(v.symbol, v))
    ...
    def test_expression_parse_negative_coefficient(self):
        ...
        self.assertEqual(e, Expression(
            {Variable('x'): -1, Variable('y'): 4, Variable('z'): -3}))
